...
stratify :: Waterfall.Solid -> Waterfall.Solid 
stratify s = s <> mconcat
    [
    let b = Waterfall.unitCube 
            & Waterfall.scale (V3 30 0.5 1) 
            & Waterfall.translate (unit _y ^* i * 3)
            & Waterfall.translate (unit _z ^* 0.01)
        in case Waterfall.axisAlignedBoundingBox (b `Waterfall.intersection` s) of 
                Nothing -> mempty 
                Just (V3 lo _ _, V3 hi _ _) -> 
                     Waterfall.unitCube 
                        & Waterfall.scale (V3 (hi-lo-2) 0.5 1) 
                        & Waterfall.translate (V3 (lo+1) (i * 3) 0.01)
    | i <- [1..15]] 
...
ornament = 
    let 
        bridge = Waterfall.unitCube 
            ...
            & Waterfall.translate (V3 15 44 0)
        hoop = Waterfall.unitCylinder 
            ...
            & Waterfall.translate (V3 15 50.5 0)
    in hoop <> bridge <> stratify elixirLogo


main :: IO ()
main = do
    Waterfall.writeSTL 0.1 "elixir-ornament.stl" ornament
